...
static void user_post_property(void)
{
    ...
    cJSON_AddItemToObject(root, "GasSensorState", cJSON_CreateNumber(CH4_Alm_status));
    cJSON_AddItemToObject(root, "CH4", cJSON_CreateNumber(CH4)); 

    wifi_ap_record_t wifidata;
    if (esp_wifi_sta_get_ap_info(&wifidata) == 0)
    {
        cJSON_AddItemToObject(root, "WiFI_RSSI", cJSON_CreateNumber(wifidata.rssi));
        cJSON_AddItemToObject(root, "WIFI_AP_BSSID", cJSON_CreateString((char *)wifidata.ssid));
    }

    char *cjson_printunformat;
    cjson_printunformat=cJSON_PrintUnformatted(root);
    memcpy(property_payload,cjson_printunformat,strlen(cjson_printunformat));
